...
public class ImapThreadRetriever {
	...
	public ThreadData retrieveThreads() throws MessagingException{
		...
		logMessage("Retrieving data from the latest "+NUM_THREADS_RETRIEVED+" threads using at most the latest "+MAX_MESSAGES+" messages.\n");
		while(true){
			int startMessage = Math.min(-1*(maxMessage-totalMessages)+1, MAX_MESSAGES);
			//int endMessage = Math.min(-1*(minMessage - totalMessages)+1, MAX_MESSAGES);
			//
			//logMessage("Retrieving messages "+(startMessage)+" to "+(endMessage) + " (newest first)");
			...
			for(int msgPos=messages.length - 1; msgPos>=0; msgPos--){
				...
				sortIntoThreads(message, messageID, references, inReplyTo, idsForThreads, threads, unseenMessages, seenMessages);
				
				if((threads.size() >= NUM_THREADS_RETRIEVED && unseenMessages.size() == 0) || seenMessages.size() >= MAX_MESSAGES){
					break;
				}
	...
	public void sortIntoThreads(Message message, String messageID, ArrayList<String> references, String inReplyTo,
			ArrayList<ArrayList<String>> idsForThreads, ArrayList<Set<Message>> threads, Set<String> unseenMessages, Set<String> seenMessages){
		...
		if(prevThread != null || added){
			...
			for(String id: references){
				if(id.startsWith("Message Email Reply")) continue;
				if(id.endsWith("groups.facebook.com>")) continue;
